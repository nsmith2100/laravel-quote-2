    ...
    <head>
        ...
        <script language="javascript">
            ...
            const clickLookUp = async () => {
                ...
                const response = await fetch('http://127.0.0.1:8000/api/auth/quote/' + document.getElementById("quote_id").value, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                ...
                const myJson = await response.json();
                console.log(myJson);
                if(myJson.quote !== undefined && myJson.quote !== null){
                    var ages_str = "";
                    myJson.ages.forEach(age => {
                        ages_str += (ages_str.length > 1 ? ", " : "") + age.age;
                    });

                    var string_temp = "Quote ID: " + myJson.quote.id + "<br />" + 
                        "Ages: " + ages_str + "<br />" +
                        "Start Date: " + myJson.quote.start_date + "<br />" +
                        "End Date: " + myJson.quote.end_date + "<br />" +
                        "Total: " + parseFloat(myJson.quote.total).toFixed(2) + " " + myJson.quote.currency_id + "<br />";

                    document.getElementById('information').innerHTML = string_temp;

                    document.getElementById('information_div').classList.add("hidden");
                    document.getElementById('information_display_div').classList.remove("hidden");
                } else {
            ...
            const clickNewQuote = async () => {
                ...
                const response = await fetch('http://127.0.0.1:8000/api/auth/quote', {
                    method: 'POST',
                    body: JSON.stringify({
    ...
    <body class="antialiased">
        <div id="body_content">
           ...
           <div id="information_div" class="hidden">
                ...
                <h2>Logout</h2>
                <div>
                    <button id="logout_button" name="logout_button" onclick="clickLogout()">Logout</button>
                </div>
                <h2>Find a Qoute</h2>
                <div>
                ...
                <div>
                    <button id="btn_lookup" name="btn_lookup" onclick="clickLookUp()">Lookup</button>
                </div>
                <h2>New Quote</h2>
                <p>If a date picker does not display please format your dates, YYYY-MM-DD</p>
                <div>
                    <div>
                        <label for="ages">Ages (Please comma separate):</label>
                        <div>
                    ...
                    <div>
                        <button id="btn_new_quote" name="btn_new_quote" onclick="clickNewQuote()">New Quote</button>
                    </div>
                </div>
           </div>
           <div id="information_display_div" class="hidden">
            <button id="button_back" name="button_back" onclick="clickBackButton()">Back</button>
